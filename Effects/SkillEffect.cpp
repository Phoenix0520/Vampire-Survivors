...
SkillEffect::SkillEffect()
{
	...
	for (UINT i = 0; i < 8; i++)
	{
		attackedID[i].resize(MAX_MOB, false);
		soundPlayed.push_back(false);
	}
}
...
void SkillEffect::UpdateSkillEffect1(Matrix V, Matrix P)
{
	...
	for (int i = 0; i < MAX_MOB; i++)
	{
		...
		if (IsMonsterInRegion(mob, 0.0f, 0.0f))
		{
			if (!attackedID[0][i])
				attackedID[0][i] = true;
			else
...
void SkillEffect::UpdateSkillEffect2(Matrix V, Matrix P)
{
	...
	for (int i = 0; i < MAX_MOB; i++)
	{
		...
		if (IsMonsterInRegion(mob, 0.0f, 1.0f))
		{
			mob->Attacked(id);
			SetActive(false);
			return;
...
void SkillEffect::UpdateSkillEffect3(Matrix V, Matrix P)
{
	...
	for (int i = 0; i < MAX_MOB; i++)
	{
		...
		if (IsMonsterInRegion(mob, 17.5f, 17.5f))
		{
			mob->Attacked(id);
			spearCount++;
